...
def get_movies_by_country(countries):
    ...
    try:
        ...
        # return [db.movies.find({"countries": {"$in": countries}, {"title": 1}).limit(1)]
...
def delete_user(email):
    """
    Given a user's email, deletes a user from the `users` collection and delete
...
def update_prefs(email, prefs=None):
    """
